...
public class Op02BrowserSmallFeatureEx extends BrowserSmallFeatureEx {
    ...
    public String getOperatorUA(String defaultUA) {
        ...
        if (Build.MODEL.contains("v3")) {          
            model = "ztemt73/v3";
            strTime = new SimpleDateFormat("MM.dd.yyyy").format(date);
        } else if (Build.MODEL.contains("75") && Build.MODEL.contains("v1")) {
            model = "eagle75/v1";
            strTime = new SimpleDateFormat("MM.dd.yyyy").format(date);
        } else if (Build.MODEL.contains("athens15") && Build.MODEL.contains("v1")) {
            model = "athens15/v1";
            strTime = new SimpleDateFormat("MM.dd.yyyy").format(date);
        } else if (Build.MODEL.contains("athens15") && Build.MODEL.contains("v2")) {
            model = "athens15/v2";
            strTime = new SimpleDateFormat("MM.dd.yyyy").format(date);
        } else if (Build.MODEL.contains("lenovo75")) {
            model = "lenovo75/v1";
            strTime = new SimpleDateFormat("MM.dd.yyyy").format(date);
        } else {
            model = Build.MODEL;
