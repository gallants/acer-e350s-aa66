...
static struct i2c_board_info __initdata i2c_bma250={ I2C_BOARD_INFO("BMA250", (0x3F>>1))};  //0x30  //longxuewei
...
struct bma250_i2c_data {
	...
	struct mutex			sensor_power_mutex;
...
static int BMA250_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	printk("BMA250_SetPowerMode ok!,enable=%d\r\n",enable);
	mdelay(20);
...
static int BMA250_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	mutex_lock(&obj->sensor_power_mutex);
	...
	mutex_unlock(&obj->sensor_power_mutex);
...
/*----------------------------------------------------------------------------*/
int bma250_gsensor_operate(void* self, uint32_t command, void* buff_in, int size_in,
		void* buff_out, int size_out, int* actualout) // auto detect
{
	int err = 0;
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			{
				value = *(int *)buff_in;
				mutex_lock(&priv->sensor_power_mutex);
				...
				{
					err = BMA250_SetPowerMode( priv->client, !((bool)atomic_read(&priv->sensor_power)));
				}
				mutex_unlock(&priv->sensor_power_mutex);
			}
			break;
...
static void bma250_early_suspend(struct early_suspend *h) 
{
	...
	atomic_set(&obj->sensor_power, false);
...
static int bma250_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = bma250_get_cust_acc_hw();  /*for auto detect feature*/
	...
	mutex_init(&obj->sensor_power_mutex);
	...
	if(err = bma250_create_attr(&bma250_init_info.platform_diver_addr->driver))  /*modify for auto detect feature*/
	{
	...
	sobj.self = obj;
    sobj.polling = 1;
    sobj.sensor_operate = bma250_gsensor_operate;  // auto detect
	...
	bma250_init_flag = 0;/* Add for auto detect feature */
	GSE_LOG("%s: OK\n", __func__);    
	return 0;
	...
	kfree(obj);
	exit:
	bma250_init_flag = -1;  /* Add for auto detect feature */
	GSE_ERR("%s: err = %d\n", __func__, err);        
	return err;
}
...
static int bma250_i2c_remove(struct i2c_client *client)
{
	...
	if(err = bma250_delete_attr(&bma250_init_info.platform_diver_addr->driver))  /*modify for auto detect feature*/
	{
...
static int  bma250_local_init(void)
{
	...
	BMA250_power(hw, 1);
	//bma250_force[0] = hw->i2c_num;  //yanfang 
	if(i2c_add_driver(&bma250_i2c_driver))
	{
		GSE_ERR("add driver error\n");
		return -1;
