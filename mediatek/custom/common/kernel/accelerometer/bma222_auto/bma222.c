...
/* Delete for auto detect feature end*/
...
/*----------------------------------------------------------------------------*/
int bma222_gsensor_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
static int bma222_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = bma222_get_cust_acc_hw();  /*for auto detect feature*/
	...
	if(err = bma222_create_attr(&bma222_init_info.platform_diver_addr->driver))  /*modify for auto detect feature*/
	{
	...
	sobj.self = obj;
    sobj.polling = 1;
    sobj.sensor_operate = bma222_gsensor_operate;  // auto detect
	...
	bma222_init_flag = 0;/* Add for auto detect feature */
	GSE_LOG("%s: OK\n", __func__);    
	return 0;
	...
	kfree(obj);
	exit:
	bma222_init_flag = -1;  /* Add for auto detect feature */
	GSE_ERR("%s: err = %d\n", __func__, err);        
	return err;
}
...
static int bma222_i2c_remove(struct i2c_client *client)
{
	...
	if(err = bma222_delete_attr(&bma222_init_info.platform_diver_addr->driver))  /*modify for auto detect feature*/
	{
...
static int  bma222_local_init(void)
{
	...
	BMA222_power(hw, 1);
	//bma222_force[0] = hw->i2c_num;  //yanfang 
	if(i2c_add_driver(&bma222_i2c_driver))
	{
		GSE_ERR("add driver error\n");
		return -1;
