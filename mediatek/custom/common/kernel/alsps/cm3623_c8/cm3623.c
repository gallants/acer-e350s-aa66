...
#if 1 //defined(ACER_C8)|| defined(SIMCOM_I5000) || defined(SIMCOM_I3000)//amy0504 
	if(power_on == on)
	{
		APS_LOG("ignore power control: %d\n", on);
	}
...
static cm3623_WriteCalibration(struct PS_CALI_DATA_STRUCT *data_cali)
{
...
/************************longxuewei add 2012-02-28 start*************************************/
		...
		case ALSPS_GET_PS_THD:    //ps_threshold
			...
			{
				ps_cali_temp.close=atomic_read(&obj->ps_thd_val);
				ps_cali_temp.far_away=atomic_read(&obj->ps_thd_val)-2;
		...
		case ALSPS_SET_PS_CALI:
			...
			cm3623_WriteCalibration(&ps_cali_temp);
...
static int cm3623_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	atomic_set(&obj->als_cmd_val, 0xD7);//DF 800ms D3 100ms  D7:200ms  DB:400ms
#else
	atomic_set(&obj->als_cmd_val, 0xD3);//DF 800ms D3 100ms  D7:200ms
#endif
...
/*
#if defined(SIMCOM_I5000) //amy0504
	  atomic_set(&obj->ps_cmd_val,  0x55);//enable ps interrupt  //0xc5:256ms  0x55:76ms
#elif defined(ACER_C8)
	  atomic_set(&obj->ps_cmd_val,  0x05);//enable ps interrupt  //0xc5:256ms;0x00:32ms
#else
